...
function Enviament() {
    ...
    if (canvi.value == 'Barbarian') {
        ...
        let rabia = document.getElementById('mal');
        let aux = new Barbarian(nom.value, punts.value, "Barbarian", arma.value, rabia.value);
        arrayJugadors.push(aux);
    }
    else {
        let nom = document.getElementById('nom');
        let punts = document.getElementById('punts');
        let conjur = document.getElementById('conjur');
        let arcana = document.getElementById('arcana');
        let aux = new Wizard(nom.value, punts.value, "Wizard", conjur.value, arcana.value);
        arrayJugadors.push(aux);
    }
    formulari2.innerHTML += '<br>';
