...
/**
 * 
 * THIS CLASS HAS BEEN REPLACED WITH THE NEW VERSION OF BOARD AND WILL NOT BE UPDATED
 * 
...
public class BoardOld extends JPanel {
	...
	private void initSquares() {
		...
		squares = new SquareOld[rows][cols];
		int w = getWidth();
		int h = getHeight();
		...
		for (int i = 0; i < rows; i++) {
			...
			for (int j = 0; j < cols; j++) {
				...
				squares[i][j] = new SquareOld(i, j, r, x, y, xInc, yInc, this);
			}
		}
	}
	...
	private void initoverSquares(int fil, int col) {
		...
		oversquares = new SquareOld[fil][col];
		int w = getWidth();
		int h = getHeight();
		...
		for (int i = 0; i < fil; i++) {
			...
			for (int j = 0; j < col; j++) {
				double x = padding + j * xInc;
				Rectangle2D.Double r = new Rectangle2D.Double(x, y, xInc, yInc);
				oversquares[i][j] = new SquareOld(i, j, r, x, y, xInc, yInc, this);
			}
		}
	}
...
class SquareOld {
	...
	public SquareOld(int f, int c, Rectangle2D.Double rect, double e, double d, double a, double b, BoardOld board) {
		row = f;
		col = c;
		x = e;
		y = d;
	...
	public void overdraw(Graphics2D g2, int value, String[] overimatges, boolean actfreedraw, double[] freedrawx,
		...
		if (!(overimatges[value].equals(""))) { // An empty string represents no image (or transparency)
			Image img = null;
			try {
				img = new ImageIcon((overimatges[value])).getImage();
				if (actfreedraw) {
					...
					g2.drawImage(img, (int) x, (int) y, (int) (x + xInc), (int) (y + yInc), 0, 0, img.getWidth(null), img.getHeight(null), board);
				}
			} catch (Exception e) { // almost every error is due to the image not being included on the project
										// folder
	...
	public void draw(Graphics2D g2, int a, boolean actcolors, int[] colors, boolean actborde, int borde, int fons,
		...
		if (actimatges) {
			if (!(imatges[value].equals(""))) { // An empty string represents no image (or transparency)
				Image img = null;
				try {
					img = new ImageIcon((imatges[value])).getImage();
					...
					if (actfreedraw) {
						...
						g2.drawImage(img, (int) x, (int) y, (int) (x + xInc), (int) (y + yInc), 0, 0, img.getWidth(null),
								img.getHeight(null), board);
		 ...
		 * 
		 * // si vols que les teves opcions vagin per ratol�, a partir d'aquesta //
		 * funci� hauries de cridar una funcio estatica de la TEVA classe. Tamb� // pots
		 * consultar les variables mitjan�ant els getters de mosuefil y // mousecol }
		 */
	}
}