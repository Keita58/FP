...
public class PowerUpDis extends PowerUp {
	
	Timer timer = new Timer();
	Sprite punts = null;

	public PowerUpDis(String name, int x1, int y1, int x2, int y2, double angle, String path, Field f) {
	...
	public void onCollisionEnter(Sprite sprite) {
		if(sprite instanceof Nau) {
			this.delete();	
			if(((Nau)sprite).mddl > 5) {
				((Nau) sprite).dis += 0.25;
				((Nau) sprite).cntr = 0;
				((Nau) sprite).mddl -= 1;
			}
			else if(!((Nau) sprite).doble) {
				((Nau) sprite).doble = true;
				((Nau) sprite).mddl = 6;
				((Nau) sprite).dis -= 1;
			}
			punts(sprite, false);
			...
			UI.instance.punts.punts();
			f.w.sfxVolume = 80;
			f.w.playSFX("galaga/Galaga - Demons of Death SFX (6).wav");
		}
	}
